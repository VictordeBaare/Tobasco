CREATE PROCEDURE [dbo].[CPK20_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK20_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK20
		   SET 
				CPK20.Training = @Training,
CPK20.Duur = @Duur,
CPK20.Kosten = @Kosten,
CPK20.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK20.ModifiedOn = SYSDATETIME(),
CPK20.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK20.Id = @Id
		   AND CPK20.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK20-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK20_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
