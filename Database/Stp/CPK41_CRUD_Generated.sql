CREATE PROCEDURE [dbo].[CPK41_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK41_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK41
		   SET 
				CPK41.Training = @Training,
CPK41.Duur = @Duur,
CPK41.Kosten = @Kosten,
CPK41.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK41.ModifiedOn = SYSDATETIME(),
CPK41.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK41.Id = @Id
		   AND CPK41.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK41-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK41_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
