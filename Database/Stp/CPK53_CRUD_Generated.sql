CREATE PROCEDURE [dbo].[CPK53_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK53_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK53
		   SET 
				CPK53.Training = @Training,
CPK53.Duur = @Duur,
CPK53.Kosten = @Kosten,
CPK53.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK53.ModifiedOn = SYSDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK53.Id = @Id
		   AND CPK53.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK53-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK53_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
