CREATE PROCEDURE [dbo].[CPK37_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK37_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK37
		   SET 
				CPK37.Training = @Training,
CPK37.Duur = @Duur,
CPK37.Kosten = @Kosten,
CPK37.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK37.ModifiedOn = SYSDATETIME(),
CPK37.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK37.Id = @Id
		   AND CPK37.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK37-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK37_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
