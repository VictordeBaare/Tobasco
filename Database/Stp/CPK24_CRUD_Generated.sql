CREATE PROCEDURE [dbo].[CPK24_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK24_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK24
		   SET 
				CPK24.Training = @Training,
CPK24.Duur = @Duur,
CPK24.Kosten = @Kosten,
CPK24.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK24.ModifiedOn = SYSDATETIME(),
CPK24.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK24.Id = @Id
		   AND CPK24.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK24-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK24_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
