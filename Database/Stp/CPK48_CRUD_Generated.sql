CREATE PROCEDURE [dbo].[CPK48_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK48_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK48
		   SET 
				CPK48.Training = @Training,
CPK48.Duur = @Duur,
CPK48.Kosten = @Kosten,
CPK48.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK48.ModifiedOn = SYSDATETIME(),
CPK48.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK48.Id = @Id
		   AND CPK48.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK48-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK48_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
