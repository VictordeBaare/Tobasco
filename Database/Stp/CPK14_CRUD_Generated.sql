CREATE PROCEDURE [dbo].[CPK14_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK14_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK14
		   SET 
				CPK14.Training = @Training,
CPK14.Duur = @Duur,
CPK14.Kosten = @Kosten,
CPK14.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK14.ModifiedOn = SYSDATETIME(),
CPK14.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK14.Id = @Id
		   AND CPK14.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK14-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK14_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
