CREATE PROCEDURE [dbo].[CPK52_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK52_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK52
		   SET 
				CPK52.Training = @Training,
CPK52.Duur = @Duur,
CPK52.Kosten = @Kosten,
CPK52.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK52.ModifiedOn = SYSDATETIME(),
CPK52.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK52.Id = @Id
		   AND CPK52.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK52-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK52_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
