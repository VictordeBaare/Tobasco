CREATE PROCEDURE [dbo].[CPK5_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK5_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK5
		   SET 
				CPK5.Training = @Training,
CPK5.Duur = @Duur,
CPK5.Kosten = @Kosten,
CPK5.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK5.ModifiedOn = SYSDATETIME(),
CPK5.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK5.Id = @Id
		   AND CPK5.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK5-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK5_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
