CREATE PROCEDURE [dbo].[CPK1_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK1_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK1
		   SET 
				CPK1.Training = @Training,
CPK1.Duur = @Duur,
CPK1.Kosten = @Kosten,
CPK1.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK1.ModifiedOn = SYSDATETIME(),
CPK1.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK1.Id = @Id
		   AND CPK1.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK1-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK1_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
