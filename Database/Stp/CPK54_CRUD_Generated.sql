CREATE PROCEDURE [dbo].[CPK54_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK54_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK54
		   SET 
				CPK54.Training = @Training,
CPK54.Duur = @Duur,
CPK54.Kosten = @Kosten,
CPK54.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK54.ModifiedOn = SYSDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK54.Id = @Id
		   AND CPK54.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK54-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK54_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
