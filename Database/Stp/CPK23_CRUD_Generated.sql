CREATE PROCEDURE [dbo].[CPK23_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK23_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK23
		   SET 
				CPK23.Training = @Training,
CPK23.Duur = @Duur,
CPK23.Kosten = @Kosten,
CPK23.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK23.ModifiedOn = SYSDATETIME(),
CPK23.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK23.Id = @Id
		   AND CPK23.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK23-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK23_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
