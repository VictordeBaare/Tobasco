CREATE PROCEDURE [dbo].[CPK8_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK8_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK8
		   SET 
				CPK8.Training = @Training,
CPK8.Duur = @Duur,
CPK8.Kosten = @Kosten,
CPK8.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK8.ModifiedOn = SYSDATETIME(),
CPK8.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK8.Id = @Id
		   AND CPK8.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK8-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK8_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
