CREATE PROCEDURE [dbo].[CPK21_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK21_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK21
		   SET 
				CPK21.Training = @Training,
CPK21.Duur = @Duur,
CPK21.Kosten = @Kosten,
CPK21.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK21.ModifiedOn = SYSDATETIME(),
CPK21.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK21.Id = @Id
		   AND CPK21.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK21-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK21_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
