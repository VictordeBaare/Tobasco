CREATE PROCEDURE [dbo].[CPK29_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK29_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK29
		   SET 
				CPK29.Training = @Training,
CPK29.Duur = @Duur,
CPK29.Kosten = @Kosten,
CPK29.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK29.ModifiedOn = SYSDATETIME(),
CPK29.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK29.Id = @Id
		   AND CPK29.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK29-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK29_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
