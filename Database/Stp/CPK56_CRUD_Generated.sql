CREATE PROCEDURE [dbo].[CPK56_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK56_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK56
		   SET 
				CPK56.Training = @Training,
CPK56.Duur = @Duur,
CPK56.Kosten = @Kosten,
CPK56.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK56.ModifiedOn = SYSDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK56.Id = @Id
		   AND CPK56.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK56-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK56_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
