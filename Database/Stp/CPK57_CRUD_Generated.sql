CREATE PROCEDURE [dbo].[CPK57_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK57_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK57
		   SET 
				CPK57.Training = @Training,
CPK57.Duur = @Duur,
CPK57.Kosten = @Kosten,
CPK57.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK57.ModifiedOn = SYSDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK57.Id = @Id
		   AND CPK57.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK57-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK57_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
