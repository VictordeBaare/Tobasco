CREATE PROCEDURE [dbo].[CPK22_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK22_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK22
		   SET 
				CPK22.Training = @Training,
CPK22.Duur = @Duur,
CPK22.Kosten = @Kosten,
CPK22.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK22.ModifiedOn = SYSDATETIME(),
CPK22.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK22.Id = @Id
		   AND CPK22.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK22-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK22_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
