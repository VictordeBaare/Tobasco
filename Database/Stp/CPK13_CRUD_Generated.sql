CREATE PROCEDURE [dbo].[CPK13_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK13_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK13
		   SET 
				CPK13.Training = @Training,
CPK13.Duur = @Duur,
CPK13.Kosten = @Kosten,
CPK13.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK13.ModifiedOn = SYSDATETIME(),
CPK13.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK13.Id = @Id
		   AND CPK13.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK13-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK13_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
