CREATE PROCEDURE [dbo].[CPK3_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK3_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK3
		   SET 
				CPK3.Training = @Training,
CPK3.Duur = @Duur,
CPK3.Kosten = @Kosten,
CPK3.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK3.ModifiedOn = SYSDATETIME(),
CPK3.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK3.Id = @Id
		   AND CPK3.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK3-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK3_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
