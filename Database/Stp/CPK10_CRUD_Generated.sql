CREATE PROCEDURE [dbo].[CPK10_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK10_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK10
		   SET 
				CPK10.Training = @Training,
CPK10.Duur = @Duur,
CPK10.Kosten = @Kosten,
CPK10.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK10.ModifiedOn = SYSDATETIME(),
CPK10.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK10.Id = @Id
		   AND CPK10.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK10-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK10_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
