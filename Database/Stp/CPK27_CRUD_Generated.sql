CREATE PROCEDURE [dbo].[CPK27_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK27_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK27
		   SET 
				CPK27.Training = @Training,
CPK27.Duur = @Duur,
CPK27.Kosten = @Kosten,
CPK27.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK27.ModifiedOn = SYSDATETIME(),
CPK27.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK27.Id = @Id
		   AND CPK27.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK27-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK27_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
