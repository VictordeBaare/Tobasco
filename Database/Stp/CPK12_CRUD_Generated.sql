CREATE PROCEDURE [dbo].[CPK12_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK12_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK12
		   SET 
				CPK12.Training = @Training,
CPK12.Duur = @Duur,
CPK12.Kosten = @Kosten,
CPK12.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK12.ModifiedOn = SYSDATETIME(),
CPK12.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK12.Id = @Id
		   AND CPK12.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK12-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK12_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
