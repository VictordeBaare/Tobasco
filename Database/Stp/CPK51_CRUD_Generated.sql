CREATE PROCEDURE [dbo].[CPK51_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK51_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK51
		   SET 
				CPK51.Training = @Training,
CPK51.Duur = @Duur,
CPK51.Kosten = @Kosten,
CPK51.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK51.ModifiedOn = SYSDATETIME(),
CPK51.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK51.Id = @Id
		   AND CPK51.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK51-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK51_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
