CREATE PROCEDURE [dbo].[CPK16_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK16_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK16
		   SET 
				CPK16.Training = @Training,
CPK16.Duur = @Duur,
CPK16.Kosten = @Kosten,
CPK16.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK16.ModifiedOn = SYSDATETIME(),
CPK16.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK16.Id = @Id
		   AND CPK16.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK16-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK16_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
