CREATE PROCEDURE [dbo].[CPK4_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK4_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK4
		   SET 
				CPK4.Training = @Training,
CPK4.Duur = @Duur,
CPK4.Kosten = @Kosten,
CPK4.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK4.ModifiedOn = SYSDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK4.Id = @Id
		   AND CPK4.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK4-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK4_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
