CREATE PROCEDURE [dbo].[CPK44_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK44_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK44
		   SET 
				CPK44.Training = @Training,
CPK44.Duur = @Duur,
CPK44.Kosten = @Kosten,
CPK44.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK44.ModifiedOn = SYSDATETIME(),
CPK44.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK44.Id = @Id
		   AND CPK44.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK44-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK44_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
