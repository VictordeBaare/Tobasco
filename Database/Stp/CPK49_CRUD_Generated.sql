CREATE PROCEDURE [dbo].[CPK49_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK49_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK49
		   SET 
				CPK49.Training = @Training,
CPK49.Duur = @Duur,
CPK49.Kosten = @Kosten,
CPK49.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK49.ModifiedOn = SYSDATETIME(),
CPK49.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK49.Id = @Id
		   AND CPK49.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK49-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK49_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
