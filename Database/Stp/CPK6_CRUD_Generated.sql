CREATE PROCEDURE [dbo].[CPK6_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK6_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK6
		   SET 
				CPK6.Training = @Training,
CPK6.Duur = @Duur,
CPK6.Kosten = @Kosten,
CPK6.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK6.ModifiedOn = SYSDATETIME(),
CPK6.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK6.Id = @Id
		   AND CPK6.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK6-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK6_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
