CREATE PROCEDURE [dbo].[CPK7_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK7_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK7
		   SET 
				CPK7.Training = @Training,
CPK7.Duur = @Duur,
CPK7.Kosten = @Kosten,
CPK7.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK7.ModifiedOn = SYSDATETIME(),
CPK7.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK7.Id = @Id
		   AND CPK7.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK7-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK7_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
