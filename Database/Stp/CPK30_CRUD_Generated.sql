CREATE PROCEDURE [dbo].[CPK30_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK30_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK30
		   SET 
				CPK30.Training = @Training,
CPK30.Duur = @Duur,
CPK30.Kosten = @Kosten,
CPK30.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK30.ModifiedOn = SYSDATETIME(),
CPK30.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK30.Id = @Id
		   AND CPK30.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK30-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK30_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
