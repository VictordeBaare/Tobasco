CREATE PROCEDURE [dbo].[CPK11_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK11_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK11
		   SET 
				CPK11.Training = @Training,
CPK11.Duur = @Duur,
CPK11.Kosten = @Kosten,
CPK11.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK11.ModifiedOn = SYSDATETIME(),
CPK11.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK11.Id = @Id
		   AND CPK11.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK11-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK11_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
