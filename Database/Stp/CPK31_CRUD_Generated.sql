CREATE PROCEDURE [dbo].[CPK31_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK31_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK31
		   SET 
				CPK31.Training = @Training,
CPK31.Duur = @Duur,
CPK31.Kosten = @Kosten,
CPK31.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK31.ModifiedOn = SYSDATETIME(),
CPK31.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK31.Id = @Id
		   AND CPK31.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK31-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK31_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
