CREATE PROCEDURE [dbo].[CPK45_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK45_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK45
		   SET 
				CPK45.Training = @Training,
CPK45.Duur = @Duur,
CPK45.Kosten = @Kosten,
CPK45.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK45.ModifiedOn = SYSDATETIME(),
CPK45.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK45.Id = @Id
		   AND CPK45.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK45-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK45_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
