CREATE PROCEDURE [dbo].[CPK55_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK55_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK55
		   SET 
				CPK55.Training = @Training,
CPK55.Duur = @Duur,
CPK55.Kosten = @Kosten,
CPK55.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK55.ModifiedOn = SYSDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK55.Id = @Id
		   AND CPK55.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK55-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK55_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
