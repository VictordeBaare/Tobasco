CREATE PROCEDURE [dbo].[CPK43_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK43_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK43
		   SET 
				CPK43.Training = @Training,
CPK43.Duur = @Duur,
CPK43.Kosten = @Kosten,
CPK43.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK43.ModifiedOn = SYSDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK43.Id = @Id
		   AND CPK43.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK43-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK43_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
