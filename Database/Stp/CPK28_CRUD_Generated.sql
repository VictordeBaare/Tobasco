CREATE PROCEDURE [dbo].[CPK28_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK28_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK28
		   SET 
				CPK28.Training = @Training,
CPK28.Duur = @Duur,
CPK28.Kosten = @Kosten,
CPK28.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK28.ModifiedOn = SYSDATETIME(),
CPK28.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK28.Id = @Id
		   AND CPK28.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK28-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK28_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
