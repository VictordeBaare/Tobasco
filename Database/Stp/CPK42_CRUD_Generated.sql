CREATE PROCEDURE [dbo].[CPK42_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK42_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK42
		   SET 
				CPK42.Training = @Training,
CPK42.Duur = @Duur,
CPK42.Kosten = @Kosten,
CPK42.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK42.ModifiedOn = SYSDATETIME(),
CPK42.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK42.Id = @Id
		   AND CPK42.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK42-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK42_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
