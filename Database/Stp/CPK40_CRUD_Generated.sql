CREATE PROCEDURE [dbo].[CPK40_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK40_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK40
		   SET 
				CPK40.Training = @Training,
CPK40.Duur = @Duur,
CPK40.Kosten = @Kosten,
CPK40.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK40.ModifiedOn = SYSDATETIME(),
CPK40.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK40.Id = @Id
		   AND CPK40.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK40-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK40_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
