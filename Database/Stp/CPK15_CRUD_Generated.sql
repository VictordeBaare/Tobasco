CREATE PROCEDURE [dbo].[CPK15_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK15_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK15
		   SET 
				CPK15.Training = @Training,
CPK15.Duur = @Duur,
CPK15.Kosten = @Kosten,
CPK15.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK15.ModifiedOn = SYSDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK15.Id = @Id
		   AND CPK15.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK15-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK15_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
