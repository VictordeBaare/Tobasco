CREATE PROCEDURE [dbo].[CPK32_Insert]
	@Id bigint output,
...
END;
GO
CREATE PROCEDURE [dbo].[CPK32_Update]
		@Id [bigint],
@Training nvarchar(100),
@Duur nvarchar(100),
@Kosten nvarchar(100),
...
BEGIN
		...
		UPDATE [dbo].CPK32
		   SET 
				CPK32.Training = @Training,
CPK32.Duur = @Duur,
CPK32.Kosten = @Kosten,
CPK32.ModifiedBy = ISNULL(@ModifiedBy, SUSER_SNAME()),
CPK32.ModifiedOn = SYSDATETIME(),
CPK32.ModifiedOnUTC = SYSUTCDATETIME()            
		OUTPUT Inserted.[RowVersion]
		 ...
		 WHERE CPK32.Id = @Id
		   AND CPK32.[RowVersion] = @RowVersion
		...
		BEGIN
            DECLARE @message AS nvarchar(2048) = CONCAT(QUOTENAME(OBJECT_SCHEMA_NAME(@@PROCID)), '.', OBJECT_NAME(@@PROCID), N': '
                                                       ...
                                                       ,N'The CPK32-row with Id=', @Id, N' and RowVersion=', CAST(@RowVersion AS binary(8)), N' cannot be altered by ', @ModifiedBy, N'. '
...
END;
GO
CREATE PROCEDURE [dbo].[CPK32_Delete]
		@Id [bigint]
       ,@RowVersion [rowversion]
       ,@ModifiedBy nvarchar(256)
AS
